 ...
 * @author Admin
 */
public class UpdateTagController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int result = tDAO.updateTag(Integer.parseInt(idRemove), name, des);
            } else {
                request.setAttribute("FAULTUPDATE", faultMess);
                TagDTO tagDTO = new TagDTO();
                tagDTO.setId(idRemove);
                tagDTO.setName(name);
                tagDTO.setDes(des);
                request.setAttribute("FAULTVALUE", tagDTO);
            }
            request.getRequestDispatcher("LoadAddTagController?PageID=1").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Bug at update tag controller");
        }
    }
