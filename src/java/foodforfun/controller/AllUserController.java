...
public class AllUserController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<AccountDTO> nextResult = postDAO.showAllUser_AdminPage(page + 1);
            request.setAttribute("NEXTRESULT", nextResult.size());
            
            List<AccountDTO> accDTO = postDAO.showAllUser_AdminPage(page);
            request.setAttribute("LISTACCOUNT", accDTO);
            url = "AllUser.jsp";
        } catch (Exception e) {
            log("ERROR at AllUserController " + e.getMessage());
        } finally {
